...
namespace Logging_App.WebService
{
    ...
    /// DataPush 的摘要说明
    /// </summary>
    public class DataPushXY : IHttpHandler, System.Web.SessionState.IReadOnlySessionState
    {
        ...
        private void DeleteLogTask(string requsition_cd, string process_id)
        {
            List<int> fileids = new List<int>();
            var scan_fileid = DbHelperOraXY.GetSingle("select REQUISITION_SCANNING_FILEID from DM_LOG_TASK where REQUISITION_CD='" + requsition_cd + "'");
            if (!string.IsNullOrWhiteSpace(Convert.ToString(scan_fileid))) fileids.Add(Convert.ToInt32(scan_fileid));

            var plan_dt = DbHelperOraXY.Query("select PLAN_CONTENT_SCANNING_FILEID from DM_LOG_OPS_PLAN  where REQUISITION_CD='" + requsition_cd + "'").Tables[0];
            var result_dt = DbHelperOraXY.Query("select FILEID  from PRO_LOG_RAPID_RESULTS  where REQUISITION_CD='" + requsition_cd + "'").Tables[0];
            var original_dt = DbHelperOraXY.Query("select FILEID  from PRO_LOG_RAPID_ORIGINAL_DATA where JOB_PLAN_CD like '" + requsition_cd + "%'").Tables[0];
            ...
            if (process_id != null)
            {
                var dt = DbHelperOraXY.Query("select fileid from sys_processing_uploadfile where process_id=:PROCESS_ID",
            ...
            {
                var upload_dt = DbHelperOraXY.Query("select c.* from sys_processing_uploadfile a,sys_file_upload b,sys_upload c where a.fileid=b.fileid and b.uploadid=c.uploadid and a.fileid=:FILEID",
                ...
                if (upload_dt.Rows.Count > 0)
                {
                    ...
                    DbHelperOraXY.ExecuteSql("delete from sys_file_upload where fileid=:FILEID",
                        ServiceUtils.CreateOracleParameter(":FILEID", OracleDbType.Decimal, fileids[i]));
                    var isExists = DbHelperOraXY.Exists("select count(1) from sys_file_upload where rownum<2 and uploadid=:UPLOADID",
                    ...
                    var dir = upload_dt.Rows[0].FieldEx<string>("PATHMAIN");
                    var fileName = string.Format("{0}-{1}-{2}", sha1, md5, length);
                    DbHelperOraXY.Query("delete from sys_upload where uploadid=:UPLOADID",
                    ServiceUtils.CreateOracleParameter(":UPLOADID", OracleDbType.Decimal, uploadid));
                    //删除ftp上文件
                    if (ftpHelper.FileExist(dir, fileName))
                    {
                        ftpHelper.DeleteFile(dir, fileName);
                    }
                }
                DbHelperOraXY.ExecuteSql("delete from sys_file_path where pathid not in(select pathid from sys_file_upload)");
            }
            ...
            strSql.Append("end;");
            DbHelperOraXY.ExecuteSqlTran(strSql.ToString(),
                ServiceUtils.CreateOracleParameter(":REQUISITION_CD", OracleDbType.Varchar2, requsition_cd),
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                string well_name = DbHelperOra.GetSingle("select well_name from com_well_basic where well_id=:well_id",
                    ServiceUtils.CreateOracleParameter("well_id", OracleDbType.Char, well_id)) as string;
                bool isExist;
                string xy_well_id = null;
                string xy_drill_job_id = null;
                string xy_process_id = null;
                decimal? xy_fileid = null;
                decimal DataSize = 0;
                DateTime StartTime = DateTime.Now;
                ftpHelper = new FtpHelper();
                Dictionary<decimal, decimal> resultDic = new Dictionary<decimal, decimal>();
                Dictionary<string, string> curveDic = new Dictionary<string, string>();
                Dictionary<decimal?, decimal?> fileDic = new Dictionary<decimal?, decimal?>();
                Dictionary<string, string> wellboreDic = new Dictionary<string, string>();
                ...
                StringBuilder strSql = new StringBuilder();
                //如果存在井次信息先删除
                isExist = DbHelperOraXY.Exists("select count(1) from dm_log_task where rownum<2 and requisition_cd=:requisition_cd",
                    ServiceUtils.CreateOracleParameter(":requisition_cd", OracleDbType.Varchar2, requisition_cd));
                if (isExist)
                {
                    WriteLine("存在相同井次,正在清理数据...");
                    var pid = DbHelperOraXY.GetSingle("select PROCESS_ID from DM_LOG_PROCESS where PROCESS_NAME=:PROCESS_NAME",
                        ServiceUtils.CreateOracleParameter(":PROCESS_NAME", OracleDbType.Varchar2, process_name)) as string;
                    DeleteLogTask(requisition_cd, pid);
                }

                WriteLine("开始推送：" + process_name);
                #region 井基本数据
                WriteLog("井基本数据");
                isExist = DbHelperOraXY.Exists("select count(1) from COM_WELL_BASIC where rownum<2 and WELL_NAME=:WELL_NAME", ServiceUtils.CreateOracleParameter(":WELL_NAME", OracleDbType.Varchar2, well_name));
                if (!isExist)
                {
                    strSql.Clear();
                    decimal? xy_fileid1 = null;
                    xy_well_id = DbHelperOraXY.GetSingle("select WELL_ID_SEQUENCE.nextval from dual").ToString();
                    ...
                    var eng_des_fileid = dt.Rows[0].Field<decimal?>("DRILL_ENG_DES_FILEID");
                    if (eng_des_fileid != null)
                    {
                        xy_fileid = DbHelperOraXY.GetSingle("select FILEID_SEQ.nextval from dual") as decimal?;
                        fileDic.Add(eng_des_fileid, xy_fileid);
                    }
                    var geo_des_fileid = dt.Rows[0].Field<decimal?>("DRILL_GEO_DES_FILEID");
                    if (geo_des_fileid != null)
                    {
                        if (!fileDic.ContainsKey(geo_des_fileid))
                        {
                            xy_fileid1 = DbHelperOraXY.GetSingle("select FILEID_SEQ.nextval from dual") as decimal?;
                            fileDic.Add(geo_des_fileid, xy_fileid1);
                        }
                        else
                            xy_fileid1 = fileDic[geo_des_fileid];
                    }
                    ...
                    strSql.Append(":WELL_ID,:SITE_ID,:WELL_NAME,:WELL_LEGAL_NAME,:WELL_UWI,:WELL_STRUCT_UNIT_DESCRIPTION,:WELL_FIELD_UNIT_NAME,:WELL_STRUCT_UNIT_NAME,:BEOG_LOCATION,:STRUCTURAL_LOCATION,:TRAVERSE_LINE_LOCATION,:SURVEY_X_AXIS,:SURVEY_Y_AXIS,:GROUND_ELEVATION,:SYSTEM_DATUM_OFFSET,:RANGER_X_AXIS,:RANGER_Y_AXIS,:MAGNETIC_DECLINATION,:CASING_TOP_DEPTH,:WELL_HEAD_LONGITUDE,:WELL_HEAD_LATITUDE,:LOC_COUNTRY,:LOC_STATE,:P_LOC_CITY,:LOC_COUNTY,:DRILL_GEO_DES,:DRILL_ENG_DES,:PART_UNITS,:WELL_HEAD_FLANGE,:DRILL_ENG_DES_FILEID,:DRILL_GEO_DES_FILEID,:WELL_DRILL_CONDITION,:BX_HEIGHT");
                    strSql.Append(") ");
                    DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                                           ServiceUtils.CreateOracleParameter(":WELL_ID", OracleDbType.Char, xy_well_id),
                        ...
                        ServiceUtils.CreateOracleParameter(":DRILL_ENG_DES_FILEID", OracleDbType.Decimal, xy_fileid),
                        ServiceUtils.CreateOracleParameter(":DRILL_GEO_DES_FILEID", OracleDbType.Decimal, xy_fileid1),
                ...
                {
                    xy_well_id = DbHelperOraXY.GetSingle("select well_id from com_well_basic where well_name=:well_name",
                        ServiceUtils.CreateOracleParameter(":well_name", OracleDbType.Varchar2, well_name)).ToString();
                }
                #endregion
                #region 井筒基本数据
                WriteLog("井筒基本数据");

                isExist = DbHelperOraXY.Exists("select count(1) from COM_WELLBORE_BASIC where rownum<2 and WELL_ID=:WELL_ID", ServiceUtils.CreateOracleParameter(":WELL_ID", OracleDbType.Varchar2, xy_well_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            var wellbore_id = dt.Rows[i].Field<string>("WELLBORE_ID");
                            var xy_wellbore_id = DbHelperOraXY.GetSingle("select WELLBORE_ID_SEQUENCE.nextval from dual").ToString();
                            wellboreDic.Add(wellbore_id, xy_wellbore_id);
                            dt.Rows[i]["WELLBORE_ID"] = xy_wellbore_id;
                        }
                        dt.AcceptChanges();
                        strSql.Clear();
                ...
                var well_job_name = dt.Rows[0].Field<string>("WELL_JOB_NAME");
                var activity_name = dt.Rows[0].Field<string>("ACTIVITY_NAME");
                var well_type = dt.Rows[0].Field<string>("WELL_TYPE");
                var well_sort = dt.Rows[0].Field<string>("WELL_SORT");
                isExist = DbHelperOraXY.Exists("select count(1) from COM_JOB_INFO where rownum<2 and WELL_JOB_NAME=:WELL_JOB_NAME and ACTIVITY_NAME=:ACTIVITY_NAME and WELL_TYPE=:WELL_TYPE and WELL_SORT=:WELL_SORT",
                    ServiceUtils.CreateOracleParameter(":WELL_JOB_NAME", OracleDbType.Varchar2, well_job_name),
                    ServiceUtils.CreateOracleParameter(":ACTIVITY_NAME", OracleDbType.Varchar2, activity_name),
                    ServiceUtils.CreateOracleParameter(":WELL_TYPE", OracleDbType.Varchar2, well_type),
                    ServiceUtils.CreateOracleParameter(":WELL_SORT", OracleDbType.Varchar2, well_sort)
                    );
                if (!isExist)
                {
                    strSql.Clear();
                    xy_drill_job_id = DbHelperOraXY.GetSingle("select DRILL_JOB_ID_SEQ.nextval from dual").ToString();
                    ...
                    strSql.Append(") ");
                    DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                                           ServiceUtils.CreateOracleParameter(":DRILL_JOB_ID", OracleDbType.Varchar2, xy_drill_job_id),
                        ServiceUtils.CreateOracleParameter(":WELL_ID", OracleDbType.Char, xy_well_id),
                ...
                {
                    xy_drill_job_id = DbHelperOraXY.GetSingle("select DRILL_JOB_ID from COM_JOB_INFO where WELL_JOB_NAME=:WELL_JOB_NAME and ACTIVITY_NAME=:ACTIVITY_NAME and WELL_TYPE=:WELL_TYPE and WELL_SORT=:WELL_SORT",
                    ServiceUtils.CreateOracleParameter(":WELL_JOB_NAME", OracleDbType.Varchar2, well_job_name),
                    ServiceUtils.CreateOracleParameter(":ACTIVITY_NAME", OracleDbType.Varchar2, activity_name),
                    ServiceUtils.CreateOracleParameter(":WELL_TYPE", OracleDbType.Varchar2, well_type),
                    ServiceUtils.CreateOracleParameter(":WELL_SORT", OracleDbType.Varchar2, well_sort)).ToString();
                }
                #endregion
                #region 测井任务通知单
                WriteLog("测井任务通知单");
                decimal? xy_items_id = null;
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_TASK where rownum<2 and REQUISITION_CD=:REQUISITION_CD", ServiceUtils.CreateOracleParameter(":REQUISITION_CD", OracleDbType.Varchar2, requisition_cd));
                if (!isExist)
                {
                    strSql.Clear();
                    xy_fileid = null;
                    ...
                    var items_id = dt.Rows[0].Field<decimal?>("PREDICTED_LOGGING_ITEMS_ID");
                    var scanning_fileid = dt.Rows[0].Field<decimal?>("REQUISITION_SCANNING_FILEID");
                    if (scanning_fileid != null)
                    {
                        if (!fileDic.ContainsKey(scanning_fileid))
                        {
                            xy_fileid = DbHelperOraXY.GetSingle("select FILEID_SEQ.nextval from dual") as decimal?;
                            fileDic.Add(scanning_fileid, xy_fileid);
                        }
                        else
                            xy_fileid = fileDic[scanning_fileid];
                    }
                    if (items_id != null) xy_items_id = DbHelperOraXY.GetSingle("select logging_items_id_sequence.nextval from dual") as decimal?;
                    ...
                    DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                        ...
                        ServiceUtils.CreateOracleParameter(":PREDICTED_LOGGING_ITEMS_ID", OracleDbType.Decimal, xy_items_id),
                        ...
                        ServiceUtils.CreateOracleParameter(":DRILL_JOB_ID", OracleDbType.Varchar2, xy_drill_job_id),
                        ServiceUtils.CreateOracleParameter(":WELL_JOB_NAME", "WELL_JOB_NAME", OracleDbType.Varchar2),
                        ServiceUtils.CreateOracleParameter(":REQUISITION_SCANNING_FILEID", OracleDbType.Decimal, xy_fileid)
              );
                }
                #endregion
                #region 计划任务书
                WriteLog("计划任务书");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_OPS_PLAN where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                    strSql.Clear();
                    xy_fileid = null;
                    ...
                    var scanning_fileid1 = dt.Rows[0].Field<decimal?>("PLAN_CONTENT_SCANNING_FILEID");
                    if (scanning_fileid1 != null)
                    {
                        if (!fileDic.ContainsKey(scanning_fileid1))
                        {
                            xy_fileid = DbHelperOraXY.GetSingle("select FILEID_SEQ.nextval from dual") as decimal?;
                            fileDic.Add(scanning_fileid1, xy_fileid);
                        }
                        else
                            xy_fileid = fileDic[scanning_fileid1];
                    }
                    ...
                    DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                        ...
                        ServiceUtils.CreateOracleParameter(":PLAN_CONTENT_SCANNING_FILEID", OracleDbType.Decimal, xy_fileid)
              );
                }
                #endregion
                #region 解释处理作业
                WriteLog("解释处理作业");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_PROCESS where rownum<2 and PROCESS_NAME=:PROCESS_NAME", ServiceUtils.CreateOracleParameter(":PROCESS_NAME", OracleDbType.Varchar2, process_name));
                if (!isExist)
                {
                    strSql.Clear();
                    xy_process_id = DbHelperOraXY.GetSingle("select PROCESS_ID_SEQ.nextval from dual").ToString();
                    ...
                    strSql.Append(") ");
                    DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                                           ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id),
                ...
                {
                    xy_process_id = DbHelperOraXY.GetSingle("select PROCESS_ID from DM_LOG_PROCESS where PROCESS_NAME=:PROCESS_NAME",
                        ServiceUtils.CreateOracleParameter(":PROCESS_NAME", OracleDbType.Varchar2, process_name)).ToString();
                }
                #endregion

                #region 地层分层数据
                WriteLog("地层分层数据");
                isExist = DbHelperOraXY.Exists("select count(1) from COM_BASE_STRATA_LAYER2 where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                isExist = DbHelperOraXY.Exists("select count(1) from COM_LOG_CEMENT_EVALUATION_INF where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            var cementid = Convert.ToDecimal(DbHelperOraXY.GetSingle("select CEMENTID_SEQ.nextval from dual"));
                            dt.Rows[i]["CEMENTID"] = cementid;
                        }
                        dt.AcceptChanges();
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #region 小队现场提交数据
                WriteLog("小队现场提交数据");
                isExist = DbHelperOraXY.Exists("select count(1) from COM_LOG_DEV_AZI where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("DEV_ID_SEQ.nextval,:PROCESS_ID,:DEP,:INCLNATION,:AZIMUTH,:NOTE");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #region 分层表
                WriteLog("分层表");
                isExist = DbHelperOraXY.Exists("select count(1) from COM_LOG_LAYER where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("LAYERID_SEQ.nextval,:PROCESS_ID,:FORMATION_NAME,:ST_DEVIATED_DEP,:EN_DEVIATED_DEP,:ST_VERTICAL_DEP,:EN_VERTICAL_DEP");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #region 解释成果表
                WriteLog("解释成果表");
                isExist = DbHelperOraXY.Exists("select count(1) from COM_LOG_RESULT where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            var resultid = dt.Rows[i].Field<decimal>("RESULTID");
                            var xy_resultid = Convert.ToDecimal(DbHelperOraXY.GetSingle("select RESULTID_SEQ.nextval from dual"));
                            resultDic.Add(resultid, xy_resultid);
                            dt.Rows[i]["RESULTID"] = xy_resultid;
                        }
                        dt.AcceptChanges();
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #region 井眼轨迹数据
                WriteLog("井眼轨迹数据");
                isExist = DbHelperOraXY.Exists("select count(1) from COM_LOG_WELL_TRAJECTORY where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("TRAJECTORYID_SEQ.nextval,:PROCESS_ID,:MD,:INCLNATION,:AZIMUTH,:E_ELEMENT,:N_ELEMENT,:TVD,:ALL_MOV,:P_XN_CLOSURE_AZIMUTH,:P_XN_CLOSED_DISTANCE,:DOG_LEG,:NOTE,:TYPE");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #region 井身结构数据
                WriteLog("井身结构数据");
                isExist = DbHelperOraXY.Exists("select count(1) from COM_WELLSTRUCTURE_DATA where rownum<2 and WELL_ID=:WELL_ID", ServiceUtils.CreateOracleParameter(":WELL_ID", OracleDbType.Varchar2, xy_well_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0 && wellboreDic.Count > 0)
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            var wellbore_id = dt.Rows[i].Field<string>("WELLBORE_ID");
                            dt.Rows[i]["WELLBORE_ID"] = wellboreDic[wellbore_id];
                        }
                        dt.AcceptChanges();
                        strSql.Clear();
                ...
                #region 小队施工_基本信息
                WriteLog("小队施工_基本信息");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_BASE where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("LOG_BASEID_SEQUENCE.nextval,:JOB_PLAN_CD,:JOB_PURPOSE,:COLLABORATION_DEPARTMENT,:CONSTRUCTION_ORG,:RECEIVED_INFORM_TIME,:P_XN_DRILL_DEPTH,:P_XN_LOG_DEPTH,:JOB_LAYER,:JOB_PROFILE_SERIES,:MAXIMUM_SLOPE,:MAXIMUM_SLOPE_DEPTH,:MUD_RESITIVITY,:MUD_TEMERATURE,:BOTTOM_TEMPERATURE,:BOTTOM_DEP,:WELLBORE_FLUID,:START_DEPTH,:END_DEPTH,:IS_COMPLETE,:LOG_TYPE,:LOG_MODE,:INSTRUMENT_COUNT,:INSTRUMENT_SUCCESS,:WAVE_SELECTION,:NOTE");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #region 井场班会
                WriteLog("井场班会");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_CLASS_MEET where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 多方联席会
                WriteLog("多方联席会");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_CONTACT_WILL where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 小队施工_井下设备
                WriteLog("小队施工_井下设备");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_DOWNHOLE_EQUIP where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append(":JOB_PLAN_CD,EQUIPID_SEQUENCE.nextval,:DEVICEACCOUNT_NAME,:DEVICEACCOUNT_ID,:TEAM,:WORKING_STATE,:FAULT_DESCRIPTION,:DOWN_WELL_SEQUENCE");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                                               ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", "JOB_PLAN_CD", OracleDbType.Varchar2),
                ...
                #region 钻井动态
                WriteLog("钻井动态");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_DRILL_STATE where rownum<2 and DRILL_JOB_ID=:DRILL_JOB_ID", ServiceUtils.CreateOracleParameter(":DRILL_JOB_ID", OracleDbType.Varchar2, xy_drill_job_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("DRILL_WELL_ID_SEQ.nextval,:DRILL_JOB_ID,:LAST_WEEK_MD,:CURRENT_WEEK_MD,:CURRENT_LAYER,:CREATE_DATE,:CREATE_MAN,:NOTE");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #region 钻井地质设计预测项目
                WriteLog("钻井地质设计预测项目");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_GEO_DES_ITEM where rownum<2 and DRILL_JOB_ID=:DRILL_JOB_ID", ServiceUtils.CreateOracleParameter(":DRILL_JOB_ID", OracleDbType.Varchar2, xy_drill_job_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("DRILL_WELL_ID_SEQ.nextval,:DRILL_JOB_ID,:LOG_MD,:LOG_INTERVAL,:LOG_LAYER,:LOG_ITEM,:LOG_SCALE,:CREATE_DATE,:CREATE_MAN,:NOTE");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                            //ServiceUtils.CreateOracleParameter(":DRILL_WELL_ID", "DRILL_WELL_ID", OracleDbType.Varchar2),
                            ServiceUtils.CreateOracleParameter(":DRILL_JOB_ID", OracleDbType.Varchar2, xy_drill_job_id),
                ...
                #region 测井项目
                WriteLog("测井项目");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_ITEMS where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 作业总结
                WriteLog("作业总结");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_JOB_SUMMAY where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 预测项目信息
                WriteLog("预测项目信息");
                isExist = DbHelperOraXY.Exists("select count(1) from dm_log_task a,DM_LOG_PREDICTED_ITEM b where A.PREDICTED_LOGGING_ITEMS_ID=B.PREDICTED_LOGGING_ITEMS_ID and ROWNUM<2 and a.requisition_cd=:REQUISITION_CD", ServiceUtils.CreateOracleParameter(":REQUISITION_CD", OracleDbType.Varchar2, requisition_cd));
                if (!isExist)
                {
                ...
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_RADIATION_STATUS where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 解释作业数据源
                WriteLog("解释作业数据源");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_SOURCE_DATA where rownum<2 and LOG_DATA_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                ...
                #region 三交会
                WriteLog("三交会");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_THREE_CROSS where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 小队施工_地面设备
                WriteLog("小队施工_地面设备");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_UP_EQUIP where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 小队施工_时效
                WriteLog("小队施工_时效");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_WORK_ANING where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 小队施工_作业明细
                WriteLog("小队施工_作业明细");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_WORK_DETAILS where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 小队施工_遇阻情况
                WriteLog("小队施工_遇阻情况");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_WORK_HOLDUP_DETAILS where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 小队施工_人员
                WriteLog("小队施工_人员");
                isExist = DbHelperOraXY.Exists("select count(1) from DM_LOG_WORK_PERSONNEL where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 钻头参数
                WriteLog("钻头参数");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_BIT_PROGRAM where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 小队施工_套管参数
                WriteLog("小队施工_套管参数");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_CASIN where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 小队施工-固井参数
                WriteLog("小队施工-固井参数");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_CEMENT where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 小队施工_取心参数
                WriteLog("小队施工_取心参数");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_CORE where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("COREID_SEQ.nextval,:JOB_PLAN_CD,:REQUISITION_CD,:UPDATE_DATE,:FORMATION_NAME,:START_DEPTH,:END_DEPTH,:CORE_WORD_DESCRIPTION,:CORE_GR_DATA,:CORE_PICTURE_DESCRIPTION,:CORE_RECOVERY");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #region 解释处理_数据发布
                WriteLog("解释处理_数据发布");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_DATA_PUBLISH where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("DATA_PUSH_ID_SEQ.nextval,:PROCESS_ID,:INTERPRET_CENTER,:LOG_START_TIME,:LOG_TOTAL_TIME,:LOST_TIME,:TEAM_ORG_ID,:LOG_SERIES_ID,:PRO_START_TIME,:P_TOTAL_TIME,:P_PROCESS_SOFTWARE,:PROCESSOR,:INTERPRETER,:P_SUPERVISOR,:RESULT_MAP_TYPE,:LOG_ORIGINALITY_DATA,:LOG_INTERPRET_REPORT,:LOG_INTERPRET_RESULT,:NOTE,:FILE_NUMBER,:P_SCENE_RATING,:P_INDOOR_RATING,:ACCEPTANCE_WAY_NAME");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #region 解释处理曲线评级
                WriteLog("解释处理曲线评级");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_PROCESSING_CURVERATING where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                ...
                #region 解释处理_图件
                WriteLog("解释处理_图件");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_PROCESS_MAP where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("MAPID_SEQ.nextval,:PROCESS_ID,:MAPS_CODING,:MAP_START_DEP,:MAP_END_DEP,:MAP_SCALE,:MAP_DATA_NAME,:MAP_PDF_SIZE,:MAP_PDF_DATA,:P_PROCESS_SOFTWARE,:MAP_OUT_DATE,:MAP_PERSON,:MAP_NUMBER,:MAP_TEMPLATE_SIZE,:MAP_TEMPLATE_DATA,:MAP_VERIFIER,:PROCESSING_ITEM_ID");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_PROCESSING_CURVE_INDEX where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            var curveid = dt.Rows[i].Field<decimal>("CURVEID");
                            var xy_curveid = DbHelperOraXY.GetSingle("select CURVEID_SEQ_2.nextval from dual").ToString();
                            curveDic.Add(curveid.ToString(), xy_curveid);
                            dt.Rows[i]["CURVEID"] = decimal.Parse(xy_curveid);
                        }
                        dt.AcceptChanges();
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                isExist = DbHelperOraXY.Exists("SELECT count(1) FROM PRO_LOG_PROCESSING_CURVE_DATA A,PRO_LOG_PROCESSING_CURVE_INDEX B WHERE ROWNUM<2 AND A.CURVEID=B.CURVEID and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {

                    dt = DbHelperOra.Query("SELECT A.*,B.DATA_STORAGE_WAY FROM PRO_LOG_PROCESSING_CURVE_DATA A,PRO_LOG_PROCESSING_CURVE_INDEX B WHERE A.CURVEID=B.CURVEID and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, process_id)).Tables[0];
                    if (dt.Rows.Count > 0 && curveDic.Count > 0)
                    {
                        strSql.Clear();
                        strSql.Append("insert into PRO_LOG_PROCESSING_CURVE_DATA(");
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            var block_size = dt.Rows[i].Field<decimal?>("BLOCK_SIZE");
                            var data_storage_way = dt.Rows[i].Field<decimal?>("DATA_STORAGE_WAY");
                            if (block_size != null)
                            {
                                if (data_storage_way.Value == 1 && block_size.Value > 0)
                                {
                                    var bytes = dt.Rows[i].Field<byte[]>("CURVE_DATA");
                                    var filepath = Encoding.UTF8.GetString(bytes);
                                    var pathmain = filepath.Split('\\')[0];
                                    var path = FileUpload.PathList.FirstOrDefault(o => o.id == 1).name;
                                    path = path.Remove(path.LastIndexOf('\\') + 1);
                                    var server_filepath = Path.Combine(path, filepath);
                                    pathDic.Add(server_filepath, pathmain);
                                    DataSize = DataSize + block_size.Value;
                                }
                            }
                            var curveid = dt.Rows[i].Field<decimal>("CURVEID").ToString();
                            dt.Rows[i]["CURVEID"] = curveDic[curveid];
                        }
                        dt.AcceptChanges();
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                                               ServiceUtils.CreateOracleParameter(":CURVEID", "CURVEID", OracleDbType.Decimal),
                ...
                WriteLog("综合解释成果曲线");
                isExist = DbHelperOraXY.Exists("select count(1) from COM_LOG_COM_CURVE_INDEX where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            var curveid = dt.Rows[i].Field<string>("CURVEID");
                            var xy_curveid = DbHelperOraXY.GetSingle("select CURVEID_SEQ_1.nextval from dual").ToString();
                            dt.Rows[i]["CURVEID"] = xy_curveid;
                            curveDic.Add(curveid, xy_curveid);
                        }
                        dt.AcceptChanges();
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                isExist = DbHelperOraXY.Exists("SELECT COUNT(1) FROM COM_LOG_COM_CURVE_DATA A,COM_LOG_COM_CURVE_INDEX B WHERE ROWNUM<2 AND A.CURVEID=B.CURVEID and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                    ...
                    {
                        strSql.Clear();
                        strSql.Append("insert into COM_LOG_COM_CURVE_DATA(");
                        strSql.Append("CURVEID,BLOCK_NUMBER,BLOCK_SIZE,CURVE_DATA");
                        strSql.Append(") values (");
                        strSql.Append(":CURVEID,:BLOCK_NUMBER,:BLOCK_SIZE,:CURVE_DATA");
                        strSql.Append(") ");
                ...
                #region 解释处理_处理项目
                WriteLog("解释处理_处理项目 ");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_PROCESSING_ITEM where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                ...
                #region 小队施工_生产参数
                WriteLog("小队施工_生产参数");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_PRODUCE where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 小队施工_测井监督
                WriteLog("小队施工_测井监督");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_PUNISH where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("PUBLISH_ID_SEQ.nextval,:JOB_PLAN_CD,:REASON,:PUNISH_DATE,:PUNISH_SUGGESTION,:IS_RECTIFY,:REVISE_DATE,:LOG_SUPERVISION_ID,:NOTE,:DOWN_WELL_SEQUENCE");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_RAPID_INFO where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_RAPID_RESULTS where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("RAPID_RESULTS_DATA_ID_SEQ.nextval,:REQUISITION_CD,:JOB_PLAN_CD,:RAPID_RESULTS_TYPE,:START_DEP,:END_DEP,:DATA,:DATA_SIZE,:NOTE,:FILENAME,:FILEID");
                        strSql.Append(") ");
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            var fileid = dt.Rows[i].Field<decimal?>("FILEID");
                            if (fileid != null)
                            {
                                if (!fileDic.ContainsKey(fileid))
                                {
                                    xy_fileid = DbHelperOraXY.GetSingle("select FILEID_SEQ.nextval from dual") as decimal?;
                                    fileDic.Add(fileid, xy_fileid);
                                }
                                else
                                    xy_fileid = fileDic[fileid];
                                dt.Rows[i]["FILEID"] = xy_fileid;
                            }
                        }
                        dt.AcceptChanges();
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #region 小队施工_远程指导
                WriteLog("小队施工_远程指导");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_REMOTE_DIRECT where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                    strSql.Clear();
                ...
                #region 小队施工_钻井液参数
                WriteLog("小队施工_钻井液参数");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_SLOP where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        strSql.Append(") values (");
                        strSql.Append("MUDID_SEQ.nextval,:JOB_PLAN_CD,:REQUISITION_CD,:UPDATE_DATE,:JTLT,:SLOP_PROPERTIES,:P_XN_MUD_DENSITY,:SLOP_PERSENT,:SLOP_PH,:DRILL_FLU_VISC,:SLOP_TEMPERATURE,:SLOP_RESISTIVITY,:DRILLING_FLUID_SALINITY,:MUD_FILTRATE_SALINITY,:CAKE_DENSITY,:API_FILTER_LOSS");
                        strSql.Append(") ");
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #region 井试油参数
                WriteLog("井试油参数");
                isExist = DbHelperOraXY.Exists("select count(1) from PRO_LOG_TESTOIL where rownum<2 and JOB_PLAN_CD=:JOB_PLAN_CD", ServiceUtils.CreateOracleParameter(":JOB_PLAN_CD", OracleDbType.Varchar2, job_plan_cd));
                if (!isExist)
                {
                ...
                #region 页岩气解释成果数据
                WriteLog("页岩气解释成果数据");
                isExist = DbHelperOraXY.Exists("select count(1) from COM_LOG_RESULT a, WL_ACH_SHALE_GAS_INTER b where A.RESULTID=B.RESULTID and rownum <2 and A.PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                    strSql.Clear();
                    strSql.Append("insert into WL_ACH_SHALE_GAS_INTER(");
                    ...
                    strSql.Append(") values (");
                    strSql.Append("SHALE_GAS_INTER_ID_SEQ.nextval,:RESULTID,:ORGANIC_CARBON_CONTEN_MIN,:ORGANIC_CARBON_CONTEN_MAX,:ALL_GAS_CONTENT_MIN,:ALL_GAS_CONTENT_MAX,:ADSORBED_GAS_CONTENT_MIN,:ADSORBED_GAS_CONTENT_MAX,:FREE_GAS_CONTENT_MIN,:FREE_GAS_CONTENT_MAX,:BRITTLENESS_INDEX_MIN,:BRITTLENESS_INDEX_MAX,:KEROGEN_MIN,:KEROGEN_MAX,:MAIN_STRESS_MAX_MIN,:MAIN_STRESS_MAX_MAX,:MAIN_STRESS_MAX_DIRECT_MIN,:MAIN_STRESS_MAX_DIRECT_MAX,:MAIN_STRESS_MIN_MIN,:MAIN_STRESS_MIN_MAX,:GAP_PRESS_MIN,:GAP_PRESS_MAX,:NOTE,:INDUCTION_DEEP_MIN,:INDUCTION_DEEP_MAX,:INDUCTION_MEDIUM_MIN,:INDUCTION_MEDIUM_MAX,:INDUCTION_SHALLOW_MIN,:INDUCTION_SHALLOW_MAX");
                    strSql.Append(") ");
                    ...
                    if (dt.Rows.Count > 0 && resultDic.Count > 0)
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            var resultid = dt.Rows[i].Field<decimal?>("RESULTID");
                            dt.Rows[i]["RESULTID"] = resultDic[resultid.Value];
                        }
                        dt.AcceptChanges();
                        DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
                ...
                #endregion
                #region 获取处理归档文件
                isExist = DbHelperOraXY.Exists("select count(1) from SYS_PROCESSING_UPLOADFILE where rownum<2 and PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, xy_process_id));
                if (!isExist)
                {
                    dt = DbHelperOra.Query("select fileid from SYS_PROCESSING_UPLOADFILE where PROCESS_ID=:PROCESS_ID", ServiceUtils.CreateOracleParameter(":PROCESS_ID", OracleDbType.Varchar2, process_id)).Tables[0];
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        var fileid = dt.Rows[i].Field<Decimal?>("FILEID");
                        if (fileDic.ContainsKey(fileid)) continue;
                        xy_fileid = DbHelperOraXY.GetSingle("select FILEID_SEQ.nextval from dual") as decimal?;
                        fileDic.Add(fileid, xy_fileid);
                    }
                }
                ...
                #region 井次所有文件数据
                WriteLog("井次所有文件数据");
                foreach (var fileId in fileDic.Keys)
                {
                    decimal? xy_uploadid = null;
                    decimal? xy_pathid = null;
                    dt = DbHelperOra.Query("select a.*,b.fileid,b.filename,c.filepath from sys_upload a,sys_file_upload b,sys_file_path c where b.fileid=:FILEID and b.uploadid=a.uploadid(+) and b.pathid=c.pathid(+)",
                        ServiceUtils.CreateOracleParameter(":FILEID", OracleDbType.Decimal, fileId)).Tables[0];
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        var sha1 = dt.Rows[i].Field<string>("sha1");
                        var md5 = dt.Rows[i].Field<string>("md5");
                        var length = dt.Rows[i].Field<decimal>("length");
                        var pathid = dt.Rows[i].Field<decimal>("pathid");
                        var pathmain = dt.Rows[i].Field<string>("pathmain");
                        var filename = dt.Rows[i].Field<string>("filename");
                        var filepath = dt.Rows[i].Field<string>("filepath");

                        isExist = DbHelperOraXY.Exists("select count(*) from sys_upload where rownum<2 and sha1=:sha1 and md5=:md5 and length=:length",
                        ...
                        if (!isExist)
                        {
                            xy_uploadid = DbHelperOraXY.GetSingle("select UPLOADID_SEQ.nextval from dual") as decimal?;
                            var path = FileUpload.PathList.FirstOrDefault(o => o.id == pathid).name;
                            path = path.Remove(path.LastIndexOf('\\') + 1);
                            var server_filepath = string.Format("{0}{1}\\{2}-{3}-{4}", path, pathmain, sha1, md5, length);
                            pathDic.Add(server_filepath, pathmain);
                            DataSize = DataSize + length;
                        }
                        else
                        {
                            xy_uploadid = DbHelperOraXY.GetSingle("select uploadid from sys_upload where sha1=:sha1 and md5=:md5 and length=:length",
                            ServiceUtils.CreateOracleParameter(":sha1", OracleDbType.Char, sha1),
                            ServiceUtils.CreateOracleParameter(":md5", OracleDbType.Varchar2, md5),
                            ServiceUtils.CreateOracleParameter(":length", OracleDbType.Decimal, length)) as decimal?;
                        }

                        if (filepath != null)
                        {
                            isExist = DbHelperOraXY.Exists("select count(*) from sys_file_path where rownum<2 and filepath=:filepath", ServiceUtils.CreateOracleParameter(":filepath", OracleDbType.Varchar2, filepath));
                            if (!isExist)
                            {
                                xy_pathid = DbHelperOraXY.GetSingle("select PATHID_SEQ.nextval from dual") as decimal?;
                                DbHelperOraXY.ExecuteSql("INSERT INTO SYS_FILE_PATH(PATHID,FILEPATH) values(:PATHID,:FILEPATH)",
                                    ServiceUtils.CreateOracleParameter(":PATHID", OracleDbType.Decimal, xy_pathid),
                                    ServiceUtils.CreateOracleParameter(":FILEPATH", OracleDbType.Varchar2, filepath));
                            }
                            else
                            {
                                xy_pathid = DbHelperOraXY.GetSingle("select pathid from sys_file_path where rownum<2 and filepath=:filepath", ServiceUtils.CreateOracleParameter(":filepath", OracleDbType.Varchar2, filepath)) as decimal?;
                            }
                        }
                        DbHelperOraXY.ExecuteSqlTran("DECLARE v_cnt NUMBER;v_cnt1 NUMBER;BEGIN SELECT COUNT(1) INTO v_cnt FROM SYS_UPLOAD WHERE rownum<2 and uploadid=:UPLOADID;SELECT COUNT(1) INTO v_cnt1 FROM SYS_FILE_UPLOAD WHERE rownum<2 and uploadid=:UPLOADID AND filename=:FILENAME AND pathid=:PATHID_1;if v_cnt=0 then INSERT INTO SYS_UPLOAD(UPLOADID,SHA1,MD5,LENGTH,PATHID,PATHMAIN) values (:UPLOADID,:SHA1,:MD5,:LENGTH,:PATHID,:PATHMAIN);end if;if v_cnt1=0 then INSERT INTO SYS_FILE_UPLOAD(FILEID,UPLOADID,FILENAME,PATHID) values(:FILEID,:UPLOADID,:FILENAME,:PATHID_1);end if;end;",
                                ServiceUtils.CreateOracleParameter(":SHA1", OracleDbType.Char, sha1),
                                ServiceUtils.CreateOracleParameter(":MD5", OracleDbType.Varchar2, md5),
                                ServiceUtils.CreateOracleParameter(":LENGTH", OracleDbType.Decimal, length),
                                ServiceUtils.CreateOracleParameter(":FILEID", OracleDbType.Decimal, fileDic[fileId]),
                                ServiceUtils.CreateOracleParameter(":UPLOADID", OracleDbType.Decimal, xy_uploadid),
                                ServiceUtils.CreateOracleParameter(":PATHID", OracleDbType.Decimal, pathid),
                                ServiceUtils.CreateOracleParameter(":PATHMAIN", OracleDbType.Varchar2, pathmain),
                                ServiceUtils.CreateOracleParameter(":FILENAME", OracleDbType.NVarchar2, filename),
                                ServiceUtils.CreateOracleParameter(":PATHID_1", OracleDbType.Decimal, xy_pathid));
                    }
                }
                #endregion
                ...
                if (!isExist)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        {
                            var fileid = dt.Rows[i].Field<Decimal?>("FILEID");
                            dt.Rows[i]["FILEID"] = fileDic[fileid];
                        }
                        dt.AcceptChanges();
                        strSql.Clear();
                ...
                //dt = DbHelperOra.Query("select a.*,b.fileid,b.filename,d.filepath from sys_upload a,sys_file_upload b,sys_processing_uploadfile c,sys_file_path d where c.process_id=:process_id and c.fileid=b.fileid and b.uploadid=a.uploadid and b.pathid=d.pathid(+)",
                //    ServiceUtils.CreateOracleParameter(":process_id", OracleDbType.Varchar2, process_id)).Tables[0];
                //for (int i = 0; i < dt.Rows.Count; i++)
                //{
                //    xy_fileid = DbHelperOraXY.GetSingle("select FILEID_SEQ.nextval from dual") as decimal?;
                ...
                //        path = path.Remove(path.LastIndexOf('\\') + 1);
                //        var server_filepath = string.Format("{0}{1}\\{2}-{3}-{4}", path, pathmain, sha1, md5, length);
                //        filepaths.Add(server_filepath, pathmain);
                //    }
                //    else
                //    {
                //        xy_uploadid = DbHelperOraXY.GetSingle("select uploadid from sys_upload where sha1=:sha1 and md5=:md5 and length=:length",
                ...
                //    var filepath = dt.Rows[0].Field<string>("filepath");
                //    decimal? xy_pathid = null;
                //    if (filepath != null)
                //    {
                //        isExist = DbHelperOraXY.Exists("select count(*) from sys_file_path where rownum<2 and filepath=:filepath", ServiceUtils.CreateOracleParameter(":filepath", OracleDbType.Varchar2, filepath));
                ...
                //    DbHelperOraXY.ExecuteSql("DECLARE v_cnt NUMBER;BEGIN SELECT COUNT(1) INTO v_cnt FROM SYS_UPLOAD WHERE rownum<2 and uploadid=:UPLOADID;if v_cnt=0 then INSERT INTO SYS_UPLOAD(UPLOADID,SHA1,MD5,LENGTH,PATHID,PATHMAIN) values (:UPLOADID,:SHA1,:MD5,:LENGTH,:PATHID,:PATHMAIN);end if;INSERT INTO SYS_FILE_UPLOAD(FILEID,UPLOADID,FILENAME,PATHID) values(:FILEID,:UPLOADID,:FILENAME,:PATHID_1);end;",
                //            ServiceUtils.CreateOracleParameter(":SHA1", OracleDbType.Char, sha1),
                //            ServiceUtils.CreateOracleParameter(":MD5", OracleDbType.Varchar2, md5),
                //            ServiceUtils.CreateOracleParameter(":LENGTH", OracleDbType.Decimal, length),
                //            ServiceUtils.CreateOracleParameter(":FILEID", OracleDbType.Decimal, xy_fileid),
                //            ServiceUtils.CreateOracleParameter(":UPLOADID", OracleDbType.Decimal, xy_uploadid),
                //            ServiceUtils.CreateOracleParameter(":PATHID", OracleDbType.Decimal, pathid),
                //            ServiceUtils.CreateOracleParameter(":PATHMAIN", OracleDbType.Varchar2, pathmain),
                //            ServiceUtils.CreateOracleParameter(":FILENAME", OracleDbType.NVarchar2, filename),
                ...
                pushWorkFlowData(5, process_id, xy_process_id);
                #endregion
                WritePushLog(process_name, user_name);

                #region 上传文件
                WriteLog("开始推送文件...");
                foreach (var fp in pathDic.Keys)
                {
                    if (!File.Exists(fp)) continue;
                    var rem_dir = pathDic[fp];
                    var rem_fn = fp.Split('\\').Last();
                    if (!ftpHelper.DirectoryExist(rem_dir))
                    {
                        ftpHelper.MakeDir(rem_dir);
                    }
                    if (!ftpHelper.FileExist(rem_dir, rem_fn))
                    {
                        ftpHelper.Upload(rem_dir, rem_fn, fp);
                    }
                }
                #endregion

                DateTime EndTime = DateTime.Now;
                var TotalSize = (DataSize / 1024 / 1024).ToString("#0.00");
                var TotalTime = ExecDateDiff(StartTime, EndTime).ToString();
                WriteLine("数据推送完成！" + "\r\n" + "数据量：" + TotalSize + "MB,耗时：" + TotalTime);
            }
            catch (Exception ex)
            {
            ...
            finally
            {
                try
                {
                    File.AppendAllText(AppDomain.CurrentDomain.BaseDirectory + "LogFile//DataPushLog//XY" + DateTime.Now.ToString("yyyy-MM-dd") + ".log", logBuilder.ToString());
                }
                catch (Exception e) { }
                isRunning = false;
            }
        ...
        private void WritePushLog(string process_name, string user_name)
        {
            DbHelperOra.ExecuteSql("Declare v_cnt NUMBER;BEGIN SELECT count(1) into v_cnt from DI_LOG_PUSH_XY where rownum<2 and NAME=:NAME;if v_cnt>0 then UPDATE DI_LOG_PUSH_XY SET OPERATOR=:OPERATOR,LOG_TIME=systimestamp where NAME=:NAME;else INSERT INTO DI_LOG_PUSH_XY(ID,NAME,OPERATOR) VALUES(DATA_PUSH_ID_SEQ.NEXTVAL,:NAME,:OPERATOR);end if;END;",
                        new OracleParameter[]{
        ...
        private void pushWorkFlowData(int type, string obj_id, string obj_id1 = null)
        {
            var temp_id = obj_id;
            if (obj_id1 != null) obj_id = obj_id1;
            StringBuilder strSql = new StringBuilder();
            bool isExist = DbHelperOraXY.Exists("select count(1) from SYS_WORK_FLOW where rownum<2 and OBJ_ID=:OBJ_ID and flow_type=" + type, ServiceUtils.CreateOracleParameter(":OBJ_ID", OracleDbType.Varchar2, obj_id));
            if (!isExist)
            {
                var dt = DbHelperOra.Query("select * from SYS_WORK_FLOW where OBJ_ID=:OBJ_ID and flow_type=" + type + " order by flow_id", ServiceUtils.CreateOracleParameter(":OBJ_ID", OracleDbType.Varchar2, temp_id)).Tables[0];
                ...
                strSql.Append(") values (");
                strSql.Append(":OBJ_ID,:SOURCE_LOGINNAME,:TARGET_LOGINNAME,:FLOW_TYPE,:FLOW_STATE,:FLOW_TIME,FLOW_ID_SEQ.nextval,:MESSAGE");
                strSql.Append(") ");
                DbHelperOraXY.InsertDataTable(strSql.ToString(), dt,
