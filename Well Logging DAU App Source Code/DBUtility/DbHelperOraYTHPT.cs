...
namespace Maticsoft.DBUtility
{
    ...
    /// 可以用户可以修改满足自己项目的需要。
    /// </summary>
    public abstract class DbHelperOraYTHPT
    {
        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.		
        public static string connectionString = PubConstant.GetConnectionString("YTHPT_ConnectionString");
        public DbHelperOraYTHPT()
        {
        }
            ...
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                using (OracleCommand cmd = new OracleCommand(SQLString, connection))
                {
                    try
                    {
                        ...
                        if (rows < 1 && SQLString.TrimEnd().ToUpper().EndsWith("END;"))
                            rows = 1;
                        return rows;
                    }
            ...
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                ...
                using (OracleTransaction trans = connection.BeginTransaction())
                {
                    using (OracleCommand cmd = new OracleCommand())
                    {
                        ...
                        {
                            trans.Rollback();//new Exception(E.Message) (OracleException E)
                            throw;
                        }
                    }
                }
        ...
        public static void ExecuteSqlTran(Dictionary<string,OracleParameter[]> SQLStringList)
        {
            using (OracleConnection conn = new OracleConnection(connectionString))
            {
                conn.Open();
                    ...
                    using (OracleCommand cmd = new OracleCommand())
                    {
                        try
                        {
                            ...
                            foreach (var myDE in SQLStringList)
                            {
                                ...
                                PrepareCommand(cmd, conn, trans, cmdText, myDE.Value);
        ...
        /// <returns></returns>
        public static int InsertDataTable(string insertSql, DataTable dt, params OracleParameter[] cmdParms)
        {
            ...
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                ...
                using (OracleTransaction trans = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        OracleAdapter.InsertCommand.BindByName = true;
                        foreach (OracleParameter op in cmdParms)
                        {
                            OracleAdapter.InsertCommand.Parameters.Add(op);
                        }
                        OracleAdapter.InsertCommand.Connection = connection;
                        OracleAdapter.InsertCommand.Transaction = trans;
                        int i = OracleAdapter.Update(dt);
                        dt.AcceptChanges();
                        trans.Commit();
                        return i;
                    }
